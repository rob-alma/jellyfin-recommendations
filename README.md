...
containing the plugin zip and a `manifest.json` whenever a tag like `v0.3.3` is pushed (or via
"Run workflow" in the Actions tab).

1. Push a tag, e.g. `git tag v0.3.3 && git push origin v0.3.3`, and wait for the "Release"
...
2. Copy every `.dll` from the publish/build output plus `meta.json` into a new folder under your
   Jellyfin server's plugin directory, e.g. `<jellyfin-config>/plugins/PersonalRecommendations_0.3.3.0/`:
